...
Paycheck Protection Program (PPP) loans were issued during the pandemic to help small businesses, but there was rampant fraud asssociated with the program. We use a PPP loan dataset that identifies loans confirmed by the justice system to have been issued under fradulent circumstances. We construct the architecture of a fully connected feed forward neural network to classify PPP loans for fraud, producing synthetic training data and using a weighted cross entropy loss function to account for class imbalances, ultimately allowing a neural network to identify fraud based upon loan attributes. We investigate neural network based anomaly detection as an alternative approach to detect fraud in this dataset, however we were unable to apply anomoly detection for a variety of reasons. For the classical neural network model, we were able to achieve acceptable accuracies that could enable increased efficiencies in fraud identification casework. 
...
We received mixed results following our methodology, with our fully connected neural network performing decently, though no better than early results achieved using random forest models [^10]. We could not get our anomaly detection model to function correctly. 
...
[^5]: https://arxiv.org/abs/1802.06360
[^6]: https://www.researchgate.net/publication/329829847_Deep_One-Class_Classification
[^7]: https://www.sba.gov/funding-programs/loans/covid-19-relief-options/paycheck-protection-program/ppp-data
[^8]: https://www.digilab.co.uk/posts/synthetic-minority-oversampling-technique
[^9]: https://imbalanced-learn.org/stable/#
[^10]: https://github.com/max-baum/cs152projectrepo; see `CompStats_ProjectWriteup` for more detail, note repo is private and access must be requested.
